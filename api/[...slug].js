...
const TMP_DB_PATH = join(TMP_ROOT, 'hrms.json');
const SOURCE_DB_PATH = join(__dirname, '..', 'database', 'hrms.json');
...
function ensureDataFile() {
  if (!existsSync(TMP_DB_PATH)) {
    ...
    const content = readFileSync(SOURCE_DB_PATH, 'utf-8');
    mkdirSync('/tmp', { recursive: true });
    writeFileSync(TMP_DB_PATH, content, 'utf-8');
  }
}

function loadData() {
  ensureDataFile();
  return JSON.parse(readFileSync(TMP_DB_PATH, 'utf-8'));
}

function saveData(data) {
  writeFileSync(TMP_DB_PATH, JSON.stringify(data, null, 2), 'utf-8');
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (pathname.startsWith('/api/employees/') && req.method === 'PUT') {
      ...
      const employee = data.employees[index];
      Object.assign(employee, { ...body, nik: employee.nik });
      saveData(data);
    ...
    if (pathname === '/api/payroll' && req.method === 'POST') {
      ...
      const monthAttendance = (data.attendance || []).filter((a) => {
        ...
        return a.employee_nik === body.employee_nik && month === body.month && year === body.year;
      });
    ...
    if (pathname === '/api/upload' && req.method === 'POST') {
      ...
      const filePath = `/tmp/${body.filename}`;
    ...
    console.error(error);
    return sendJSON(res, 500, { success: false, error: 'Terjadi kesalahan server.' });
  }
};
